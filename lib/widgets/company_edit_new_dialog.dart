...
class _CompanyEditDialogState extends State<CompanyEditDialog>
  ...
  void _initializeFormData() {
    ...
    // Access the CompanyModel from the companyData field
    final companyModel = widget.companyData['companyData'] as CompanyModel?;

    _orgNameController.text =
        companyModel?.displayOrgName ??
        widget.companyData['clientName']?.toString() ??
        '';
    _level1Controller.text =
        companyModel?.cooldownPeriod1?.toString() ??
        widget.companyData['cooldownPeriod1']?.toString() ??
        '';
    _level2Controller.text =
        companyModel?.cooldownPeriod2?.toString() ??
        widget.companyData['cooldownPeriod2']?.toString() ??
        '';
    _level3Controller.text =
        companyModel?.cooldownPeriod3?.toString() ??
        widget.companyData['cooldownPeriod3']?.toString() ??
        '';
  ...
  Future<Map<String, dynamic>> _sendEditCompanyRequest(
    int clientId,
    int cooldownPeriod1,
    int cooldownPeriod2,
    int cooldownPeriod3,
    ...
    try {
      final success = await CompanyService.editCompanyCooldown(
        ...
        cooldownPeriod1: cooldownPeriod1.toString(),
        cooldownPeriod2: cooldownPeriod2.toString(),
        cooldownPeriod3: cooldownPeriod3.toString(),
      );

      return {
        'success': success,
        'message': success ? 'Success' : 'Failed to update',
        'statusCode': success ? 200 : 400,
      };
    } catch (e) {
      print('Error sending edit company request: $e');
      return {
        'success': false,
  ...
  Future<void> _handleSubmit() async {
    ...
    try {
      final result = await _sendEditCompanyRequest(
        widget.companyData['clientId'] ??
            int.parse(widget.companyData['clientId'].toString()),
        level1,
        level2,
        level3,
      );
      ...
      if (result['success']) {
        ...
        _showCustomSnackBar(
          message: 'Client cooldown periods updated successfully',
          isSuccess: true,
        );
        widget.onCompanyUpdated(); // Call the callback
        Navigator.of(context).pop(true); // Return true to indicate success
      } else {
        HapticFeedback.lightImpact();
        String errorMessage =
            'Failed to update client cooldown periods. Please try again.';
          ...
          decoration: InputDecoration(
            ...
            enabledBorder: OutlineInputBorder(
              ...
              borderSide: BorderSide(
                color: readOnly ? Color(0xFFF3F4F6) : Color(0xFFE5E7EB),
                width: 1,
              ),
            ),
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Text(
                                    'Update cooldown periods for ${widget.companyData['clientName'] ?? 'Company'}',
                                    style: const TextStyle(
                                      fontSize: 14,
                                      color: Color(0xFF6B7280),
                                    ),
